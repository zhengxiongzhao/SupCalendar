    ...
    <>
      ...
      <Main>
        <div className='mb-6 flex items-center justify-between'>
          <div>
            <h1 className='text-2xl font-bold tracking-tight'>日历视图 31 · 涟漪日历</h1>
            <p className='mt-1 text-sm text-muted-foreground'>同心圆涟漪效果月历</p>
          </div>
        </div>
        ...
        <div className='overflow-hidden rounded-2xl border border-border/40 bg-card shadow-sm'>
          ...
          <div className='grid grid-cols-7 gap-1 p-3'>
            ...
            {days.map((day) => {
              ...
              const ringCount = Math.min(dayRecords.length, 4)
              return (
                <button key={dateKey} onClick={() => handleDayClick(day)} className={cn('group relative flex aspect-square flex-col items-center justify-center rounded-lg transition-all duration-200', isCurrentDay && 'bg-primary/10', !isCurrentDay && dayRecords.length > 0 && 'bg-accent/10', !isCurrentDay && dayRecords.length === 0 && 'hover:bg-accent/20', 'focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring')}>
                  {dayRecords.length > 0 && Array.from({ length: ringCount }).map((_, r) => (
                    <span key={r} className='absolute inset-0 rounded-lg border border-primary/10 animate-pulse' style={{ margin: `${r * 3}px`, opacity: 1 - r * 0.2, animationDelay: `${r * 0.3}s`, animationDuration: '3s' }} />
                  ))}
                  <span className={cn('relative flex h-7 w-7 items-center justify-center rounded-full text-xs font-medium z-10', isCurrentDay ? 'bg-primary text-primary-foreground font-bold shadow-sm' : 'text-foreground/70')}>{fmt(day, 'd')}</span>
                  {dayRecords.length > 0 && (
                    <div className='relative mt-0.5 flex gap-0.5 z-10'>
