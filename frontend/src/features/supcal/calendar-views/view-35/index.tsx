...
import { ChevronLeft, ChevronRight, CalendarDays, TrendingUp, TrendingDown, Wallet, Bell, Clock, Columns3 } from 'lucide-react'
    ...
    <>
      ...
      <Main>
        <div className='mb-6 flex items-center justify-between'>
          <div>
            <h1 className='text-2xl font-bold tracking-tight'>日历视图 35 · 双列表格</h1>
            <p className='mt-1 text-sm text-muted-foreground'>月历网格 + 收支双列表格并排展示</p>
          </div>
        </div>
          ...
          <div className='flex gap-6'>
            ...
            <div className='min-w-0 flex-1'>
              ...
              <div className='grid grid-cols-2 gap-4'>
                <div className='rounded-xl border border-emerald-500/20 bg-card shadow-sm overflow-hidden'>
                  <div className='flex items-center gap-2 bg-emerald-500/5 px-4 py-2.5 border-b border-emerald-500/15'>
                    <TrendingUp className='h-4 w-4 text-emerald-600 dark:text-emerald-400' />
                    <span className='text-sm font-semibold text-emerald-700 dark:text-emerald-400'>收入</span>
                    <Badge className='bg-emerald-500/15 text-emerald-700 dark:text-emerald-400 text-[9px] ml-auto'>{incomeRecords.length} 笔</Badge>
                  </div>
                  {incomeRecords.length > 0 ? (
                    <div className='divide-y divide-border/15'>
                      {incomeRecords.map((record) => {
                        ...
                        return (
                          <button key={record.id} onClick={() => handleDayClick(selectedDate!)} className='flex w-full items-center gap-3 px-4 py-2.5 text-left transition-all hover:bg-emerald-500/5'>
                            <div className='flex h-7 w-7 items-center justify-center rounded-md bg-emerald-500/10'>
                              <TrendingUp className='h-3.5 w-3.5 text-emerald-600 dark:text-emerald-400' />
                            </div>
                            <div className='min-w-0 flex-1'>
                              <span className='truncate text-xs font-medium block'>{record.name}</span>
                            </div>
                            <span className='text-xs font-bold text-emerald-600 dark:text-emerald-400 shrink-0'>+{formatAmount(payment.amount, payment.currency)}</span>
                          </button>
                        )
                      })}
                    </div>
                  ) : (
                    <div className='py-8 text-center text-xs text-muted-foreground/40'>暂无收入</div>
                  )}
                </div>
                <div className='rounded-xl border border-rose-500/20 bg-card shadow-sm overflow-hidden'>
                  <div className='flex items-center gap-2 bg-rose-500/5 px-4 py-2.5 border-b border-rose-500/15'>
                    <TrendingDown className='h-4 w-4 text-rose-600 dark:text-rose-400' />
                    <span className='text-sm font-semibold text-rose-700 dark:text-rose-400'>支出</span>
                    <Badge className='bg-rose-500/15 text-rose-700 dark:text-rose-400 text-[9px] ml-auto'>{expenseRecords.length} 笔</Badge>
                  </div>
                  {expenseRecords.length > 0 ? (
                    <div className='divide-y divide-border/15'>
                      {expenseRecords.map((record) => {
                        const payment = record as PaymentRecord
                        return (
                          <button key={record.id} onClick={() => handleDayClick(selectedDate!)} className='flex w-full items-center gap-3 px-4 py-2.5 text-left transition-all hover:bg-rose-500/5'>
                            ...
                            <span className='text-xs font-bold text-rose-600 dark:text-rose-400 shrink-0'>-{formatAmount(payment.amount, payment.currency)}</span>
                          </button>
                        )
                      })}
                    </div>
                  ) : (
                    <div className='py-8 text-center text-xs text-muted-foreground/40'>暂无支出</div>
                  )}
                </div>
              </div>
              {reminderRecords.length > 0 && (
                <div className='mt-4 rounded-xl border border-blue-500/20 bg-card shadow-sm overflow-hidden'>
                  <div className='flex items-center gap-2 bg-blue-500/5 px-4 py-2.5 border-b border-blue-500/15'>
                    <Bell className='h-4 w-4 text-blue-600 dark:text-blue-400' />
                    <span className='text-sm font-semibold text-blue-700 dark:text-blue-400'>提醒</span>
                    <Badge className='bg-blue-500/15 text-blue-700 dark:text-blue-400 text-[9px] ml-auto'>{reminderRecords.length} 条</Badge>
                  </div>
                  <div className='divide-y divide-border/15'>
                    {reminderRecords.map((record) => (
                      <button key={record.id} onClick={() => handleDayClick(selectedDate!)} className='flex w-full items-center gap-3 px-4 py-2.5 text-left transition-all hover:bg-blue-500/5'>
                        ...
                        <Clock className='h-3.5 w-3.5 shrink-0 text-muted-foreground/30' />
                      </button>
                    ))}
                  </div>
                </div>
              ...
              {selectedDayRecords.length === 0 && (
                <div className='flex flex-col items-center justify-center rounded-xl border border-dashed border-border/40 py-16 text-muted-foreground'>
                  ...
                  <p className='mt-1 text-xs text-muted-foreground/50'>点击左侧日期查看对应日程</p>
                </div>
              )}
            </div>
          </div>
