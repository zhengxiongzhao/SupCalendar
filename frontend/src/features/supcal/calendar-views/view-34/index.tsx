...
import { ChevronLeft, ChevronRight, CalendarDays, TrendingUp, TrendingDown, Wallet, Table2, FileText } from 'lucide-react'
...
export function CalendarView34() {
  ...
  const summaryRows = useMemo(() => {
    const rows: { date: Date; dateKey: string; income: number; expense: number; recordCount: number }[] = []
    recordsByDate.forEach((recs, key) => {
      const d = new Date(key)
      let income = 0
      let expense = 0
      ...
      rows.push({ date: d, dateKey: key, income, expense, recordCount: recs.length })
    })
    ...
    <>
      ...
      <Main>
        <div className='mb-6 flex items-center justify-between'>
          <div>
            <h1 className='text-2xl font-bold tracking-tight'>日历视图 34 · 摘要月历</h1>
            <p className='mt-1 text-sm text-muted-foreground'>月历网格 + 紧凑日摘要表格</p>
          </div>
        </div>
            ...
            <div className='min-w-0 flex-1'>
              <div className='mb-4 flex items-center justify-between rounded-xl border border-border/60 bg-card px-4 py-3 shadow-sm'>
                <div className='flex items-center gap-3'>
                  ...
                  <div>
                    <h3 className='text-sm font-semibold'>{selectedDate ? fmt(selectedDate, 'M月d日 EEEE', { locale: zhCN }) : '日程'}</h3>
                  </div>
                </div>
              ...
              <div className='rounded-xl border border-border/40 bg-card shadow-sm overflow-hidden'>
                <div className='flex items-center gap-2 px-4 py-3 border-b border-border/30'>
                  ...
                  <span className='text-sm font-semibold'>本月摘要表</span>
                  <Badge variant='secondary' className='text-[9px]'>{summaryRows.length} 天</Badge>
                </div>
                <div className='overflow-x-auto'>
                  <table className='w-full'>
                    <thead>
                      <tr className='border-b border-border/20 text-[10px] text-muted-foreground/60'>
                    ...
                    <tbody>
                      {summaryRows.map((row) => {
                        const isSel = selectedDate ? isSameDay(row.date, selectedDate) : false
                        return (
                          <tr key={row.dateKey} className={cn('border-b border-border/10 transition-colors cursor-pointer', isSel ? 'bg-primary/5' : 'hover:bg-accent/20')} onClick={() => handleDaySelect(row.date)}>
                            <td className='px-3 py-1.5 text-xs font-medium'>{fmt(row.date, 'M/d EEE', { locale: zhCN })}</td>
                            <td className='px-3 py-1.5 text-right text-xs text-emerald-600 dark:text-emerald-400 font-medium'>{row.income > 0 ? `+${formatAmount(row.income, 'CNY')}` : '-'}</td>
                            <td className='px-3 py-1.5 text-right text-xs text-rose-600 dark:text-rose-400 font-medium'>{row.expense > 0 ? `-${formatAmount(row.expense, 'CNY')}` : '-'}</td>
                            <td className='px-3 py-1.5 text-right text-xs text-muted-foreground'>{row.recordCount}</td>
                          </tr>
                        )
                      })}
                    </tbody>
                    <tfoot>
                      <tr className='border-t border-border/30 bg-muted/10'>
                        <td className='px-3 py-2 text-xs font-bold'>合计</td>
                        <td className='px-3 py-2 text-right text-xs font-bold text-emerald-600 dark:text-emerald-400'>{formatAmount(summary.income, 'CNY')}</td>
                        <td className='px-3 py-2 text-right text-xs font-bold text-rose-600 dark:text-rose-400'>{formatAmount(summary.expense, 'CNY')}</td>
                        <td className='px-3 py-2 text-right text-xs font-bold text-muted-foreground'>{allMonthRecords.length}</td>
                      </tr>
                    </tfoot>
                  </table>
                </div>
