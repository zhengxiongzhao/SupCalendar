...
export function CalendarView33() {
  ...
  const maxRecords = useMemo(() => {
    let m = 0
    recordsByDate.forEach(r => { if (r.length > m) m = r.length })
    return m || 1
  }, [recordsByDate])
  ...
  const waveformPath = useMemo(() => {
    const parts = days.map((day, i) => {
      ...
      const h = (recs.length / maxRecords) * 60 + 10
      const x = (i / (days.length - 1)) * 100
      return `L ${x} ${70 - h}`
    })
    return `M 0 70 ${parts.join(' ')} L 100 70 Z`
  }, [days, recordsByDate, maxRecords])
    ...
    <>
      ...
      <Main>
        <div className='mb-6 flex items-center justify-between'>
          <div>
            <h1 className='text-2xl font-bold tracking-tight'>日历视图 33 · 音频波形</h1>
            <p className='mt-1 text-sm text-muted-foreground'>波形可视化 + 播放器风格</p>
          </div>
        </div>
        ...
        <div className='rounded-2xl border border-border/40 bg-card shadow-sm p-4'>
          <svg viewBox='0 0 100 70' className='w-full h-32' preserveAspectRatio='none'>
            <path d={waveformPath} fill='hsl(var(--primary) / 0.15)' stroke='hsl(var(--primary))' strokeWidth='0.5' />
          </svg>
        </div>

        <div className='mt-4 overflow-hidden rounded-2xl border border-border/40 bg-card shadow-sm'>
          ...
          <div className='grid grid-cols-7 gap-1 p-3'>
            ...
            {days.map((day) => {
              ...
              const barHeight = Math.max(4, (dayRecords.length / maxRecords) * 100)
              return (
                <button key={dateKey} onClick={() => handleDayClick(day)} className={cn('group relative flex aspect-square flex-col items-center justify-end rounded-lg transition-all duration-200 overflow-hidden', isCurrentDay && 'bg-primary/10 ring-2 ring-primary/30', !isCurrentDay && dayRecords.length > 0 && 'bg-accent/20', !isCurrentDay && dayRecords.length === 0 && 'hover:bg-accent/10', 'focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring')}>
                  <div className='absolute bottom-0 w-full bg-primary/20 rounded-b-lg transition-all' style={{ height: `${barHeight}%` }} />
                  <span className={cn('relative z-10 flex h-7 w-7 items-center justify-center rounded-full text-xs font-medium', isCurrentDay ? 'bg-primary text-primary-foreground font-bold shadow-sm' : 'text-foreground/70')}>{fmt(day, 'd')}</span>
                  {dayRecords.length > 0 && (
