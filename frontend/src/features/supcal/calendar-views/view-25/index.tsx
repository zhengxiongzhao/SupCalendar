...
import { ChevronLeft, ChevronRight, CalendarDays, TrendingUp, TrendingDown, Wallet, Bell, Clock, Sun, Moon } from 'lucide-react'
...
export function CalendarView25() {
  ...
  const weekdayRecords = useMemo(() => {
    return selectedDayRecords.filter(() => {
    ...
    <>
      ...
      <Main>
        <div className='mb-6 flex items-center justify-between'>
          <div>
            <h1 className='text-2xl font-bold tracking-tight'>日历视图 25 · 周末高亮</h1>
            <p className='mt-1 text-sm text-muted-foreground'>周末列高亮显示，工作日与周末日程分区展示</p>
          </div>
        </div>
            ...
            <div className='w-[380px] shrink-0 space-y-4'>
              ...
              <div className='rounded-xl border border-border/40 bg-card p-3 shadow-sm'>
                ...
                <div className='grid grid-cols-7 gap-0.5'>
                  {Array.from({ length: firstDayOffset }).map((_, i) => {
                    const colIdx = i % 7
                    return <div key={`e-${i}`} className={cn('h-10', (colIdx === 0 || colIdx === 6) && 'bg-rose-500/5 rounded-md')} />
                  })}
                  {days.map((day) => {
                    ...
                    return (
                      <button
                        ...
                        className={cn(
                          ...
                          isSelected ? 'bg-primary text-primary-foreground font-bold shadow-sm' : isCurrentDay ? 'bg-primary/10 text-primary font-semibold' : weekend ? 'bg-rose-500/8 text-rose-600/80 hover:bg-rose-500/15' : 'text-foreground/70 hover:bg-accent/50'
                        )}
                      >
                        {fmt(day, 'd')}
              ...
              <div className='rounded-xl border border-border/40 bg-card p-3 shadow-sm'>
                <div className='flex items-center gap-2 mb-3'>
                  ...
                  <span className='text-xs font-semibold'>月度概览</span>
                </div>
                <div className='grid grid-cols-3 gap-2'>
                  <div className='rounded-lg border border-emerald-500/20 bg-emerald-500/5 px-2 py-1.5'>
                    ...
                    <p className='text-[11px] font-semibold text-emerald-700 dark:text-emerald-400'>{formatAmount(summary.income, 'CNY')}</p>
                  </div>
                  <div className='rounded-lg border border-rose-500/20 bg-rose-500/5 px-2 py-1.5'>
                    ...
                    <p className='text-[11px] font-semibold text-rose-700 dark:text-rose-400'>{formatAmount(summary.expense, 'CNY')}</p>
                  </div>
                  <div className='rounded-lg border border-primary/20 bg-primary/5 px-2 py-1.5'>
                    ...
                    <p className={cn('text-[11px] font-semibold', summary.balance >= 0 ? 'text-emerald-700 dark:text-emerald-400' : 'text-rose-700 dark:text-rose-400')}>{summary.balance >= 0 ? '+' : ''}{formatAmount(Math.abs(summary.balance), 'CNY')}</p>
                  </div>
                </div>
              </div>
            </div>
              ...
              <div className='rounded-xl border border-border/60 bg-card px-4 py-3 shadow-sm'>
                <div className='flex items-center justify-between'>
                  <div className='flex items-center gap-3'>
                    {selectedDate && isWeekend(selectedDate) ? <Moon className='h-4 w-4 text-rose-500' /> : <Sun className='h-4 w-4 text-amber-500' />}
                    <div>
                      <h3 className='text-sm font-semibold'>{selectedDate ? fmt(selectedDate, 'yyyy年M月d日 EEEE', { locale: zhCN }) : ''}</h3>
                      <p className='text-[11px] text-muted-foreground'>{selectedDate && isWeekend(selectedDate) ? '周末 · 休息日' : '工作日'}</p>
                    </div>
                  </div>
              ...
              {selectedDate && !isWeekend(selectedDate) && (
                <div>
                  <div className='mb-2 flex items-center gap-2'>
                    ...
                    <span className='text-xs font-semibold text-amber-600 dark:text-amber-400'>工作日记录</span>
                    <Badge variant='outline' className='text-[9px]'>{weekdayRecords.length}</Badge>
                  </div>
                  {weekdayRecords.length > 0 ? (
                    <div className='space-y-2'>
                      {weekdayRecords.map((record) => {
                    ...
                    <div className='rounded-xl border border-dashed border-border/40 py-8 text-center text-sm text-muted-foreground'>暂无工作日记录</div>
                  )}
                </div>
              )}
              {selectedDate && isWeekend(selectedDate) && (
                <div>
                  <div className='mb-2 flex items-center gap-2'>
                    ...
                    <span className='text-xs font-semibold text-rose-600 dark:text-rose-400'>周末记录</span>
                    <Badge variant='outline' className='text-[9px]'>{weekendRecords.length}</Badge>
                  </div>
                  {weekendRecords.length > 0 ? (
                    ...
                    <div className='rounded-xl border border-dashed border-border/40 py-8 text-center text-sm text-muted-foreground'>暂无周末记录</div>
                  )}
                </div>
              )}
