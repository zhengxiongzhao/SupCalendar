    ...
    <>
      ...
      <Main>
        <div className='mb-6 flex items-center justify-between'>
          <div>
            <h1 className='text-2xl font-bold tracking-tight'>日历视图 24 · 菱形日历</h1>
            <p className='mt-1 text-sm text-muted-foreground'>45度旋转菱形网格</p>
          </div>
        </div>
        ...
        <div className='flex justify-center overflow-x-auto py-4'>
          <div className='inline-grid grid-cols-7 gap-2' style={{ transform: 'rotate(-45deg)' }}>
            {Array.from({ length: firstDayOffset }).map((_, i) => (
              <div key={`e-${i}`} className='h-12 w-12' />
            ))}
            {days.map((day) => {
              ...
              return (
                <button key={dateKey} onClick={() => handleDayClick(day)} className={cn('flex h-12 w-12 flex-col items-center justify-center border transition-all duration-200', isCurrentDay ? 'border-primary bg-primary/20 shadow-md' : dayRecords.length > 0 ? 'border-accent/60 bg-accent/20' : 'border-border/20 bg-card hover:border-border/40', 'focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring')}>
                  <span className={cn('text-[11px] font-medium', isCurrentDay ? 'text-primary font-bold' : 'text-foreground/60')} style={{ transform: 'rotate(45deg)' }}>
                    {fmt(day, 'd')}
                  </span>
                  {dayRecords.length > 0 && (
                    <div className='flex gap-0.5 mt-0.5' style={{ transform: 'rotate(45deg)' }}>
