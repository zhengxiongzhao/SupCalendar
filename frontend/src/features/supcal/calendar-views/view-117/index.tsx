...
export function CalendarView117() {
    ...
    <>
      ...
      <Main>
        <div className='mb-6 flex items-center justify-between'>
          <div>
            <h1 className='text-2xl font-bold tracking-tight'>日历视图 117 · 花瓣月历</h1>
            <p className='mt-1 text-sm text-muted-foreground'>花瓣形状日期格，花朵绽放效果</p>
          </div>
        </div>
        {recordsQuery.isLoading ? (
          ...
          <div className='flex gap-6'>
            <div className='w-[340px] shrink-0 space-y-4'>
              <div className='flex items-center justify-between rounded-2xl border border-border/60 bg-card px-4 py-3 shadow-md'>
              ...
              <div className='rounded-lg bg-emerald-500/5 px-3 py-2'><p className='text-[10px] text-muted-foreground'>收入</p><p className='text-sm font-semibold text-emerald-600 dark:text-emerald-400'>{formatAmount(summary.income, 'CNY')}</p></div>
              <div className='rounded-lg bg-rose-500/5 px-3 py-2'><p className='text-[10px] text-muted-foreground'>支出</p><p className='text-sm font-semibold text-rose-600 dark:text-rose-400'>{formatAmount(summary.expense, 'CNY')}</p></div>
              <div className='rounded-lg bg-primary/5 px-3 py-2'><p className='text-[10px] text-muted-foreground'>结余</p><p className={cn('text-sm font-semibold', summary.balance >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400')}>{summary.balance >= 0 ? '+' : ''}{formatAmount(Math.abs(summary.balance), 'CNY')}</p></div>
            </div>
                  ...
                  {days.map((day) => {
                    ...
                    return (
                      <button
                        ...
                        className={cn(
                          'relative flex h-10 w-full flex-col items-center justify-center rounded-full text-xs transition-all',
            ...
            <div className='min-w-0 flex-1 space-y-4'>
              <div className='rounded-2xl border border-border/60 bg-card px-4 py-3 shadow-md'>
                <div className='flex items-center justify-between'>
                  <div className='flex items-center gap-3'>
                    <CalendarDays className='h-4 w-4 text-primary' />
                    <div>
                      <h3 className='text-sm font-semibold'>{selectedDate ? fmt(selectedDate, 'yyyy年M月d日 EEEE', { locale: zhCN }) : ''}</h3>
                      <p className='text-[11px] text-muted-foreground'>花瓣形状日期格，花朵绽放效果</p>
                    </div>
                  </div>
