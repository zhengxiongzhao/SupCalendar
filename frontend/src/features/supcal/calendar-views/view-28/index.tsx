...
import { ChevronLeft, ChevronRight, CalendarDays, TrendingUp, TrendingDown, Wallet, ArrowRight } from 'lucide-react'
...
export function CalendarView28() {
  ...
  const recentRecords = useMemo(() => {
    const sorted = [...allMonthRecords].slice(0, 8)
    return sorted
  }, [allMonthRecords])
    ...
    <>
      ...
      <Main>
        <div className='mb-6 flex items-center justify-between'>
          <div>
            <h1 className='text-2xl font-bold tracking-tight'>日历视图 28 · 气泡月历</h1>
            <p className='mt-1 text-sm text-muted-foreground'>圆角气泡日期格子 + 横向滚动记录条</p>
          </div>
        </div>
        {recordsQuery.isLoading ? (
          ...
          <div className='space-y-4'>
            ...
            <div className='rounded-xl border border-border/40 bg-gradient-to-br from-card to-accent/20 p-5 shadow-sm'>
              ...
              <div className='grid grid-cols-7 gap-2'>
                {Array.from({ length: firstDayOffset }).map((_, i) => <div key={`e-${i}`} className='h-11' />)}
                {days.map((day) => {
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        'relative flex h-11 w-full flex-col items-center justify-center rounded-2xl text-xs transition-all',
                        isSelected
                          ? 'bg-primary text-primary-foreground font-bold shadow-lg shadow-primary/25 scale-105'
                          : isCurrentDay
                            ? 'bg-primary/15 text-primary font-semibold ring-1 ring-primary/30'
                            : weekend
                              ? 'bg-rose-500/8 text-rose-600/80 hover:bg-rose-500/15'
                              : 'bg-accent/30 text-foreground/70 hover:bg-accent/60'
                      )}
                    >
                      <span className={cn('leading-none', isSelected && 'drop-shadow-sm')}>{fmt(day, 'd')}</span>
                      {recCount > 0 && !isSelected && (
                        <span className='mt-0.5 flex items-center justify-center rounded-full bg-primary h-3.5 min-w-3.5 px-1 text-[8px] font-bold text-primary-foreground'>
                          {recCount}
                        </span>
                      )}
                    </button>
            ...
            <div className='grid grid-cols-3 gap-3'>
              <div className='flex items-center gap-2 rounded-xl border border-emerald-500/20 bg-emerald-500/5 px-4 py-2.5'>
                ...
                <div><p className='text-[10px] text-muted-foreground'>收入</p><p className='text-xs font-bold text-emerald-700 dark:text-emerald-400'>{formatAmount(summary.income, 'CNY')}</p></div>
              </div>
              <div className='flex items-center gap-2 rounded-xl border border-rose-500/20 bg-rose-500/5 px-4 py-2.5'>
                <TrendingDown className='h-4 w-4 text-rose-600 dark:text-rose-400' />
                <div><p className='text-[10px] text-muted-foreground'>支出</p><p className='text-xs font-bold text-rose-700 dark:text-rose-400'>{formatAmount(summary.expense, 'CNY')}</p></div>
              </div>
              <div className='flex items-center gap-2 rounded-xl border border-primary/20 bg-primary/5 px-4 py-2.5'>
                <Wallet className='h-4 w-4 text-primary' />
                <div><p className='text-[10px] text-muted-foreground'>结余</p><p className={cn('text-xs font-bold', summary.balance >= 0 ? 'text-emerald-700 dark:text-emerald-400' : 'text-rose-700 dark:text-rose-400')}>{summary.balance >= 0 ? '+' : ''}{formatAmount(Math.abs(summary.balance), 'CNY')}</p></div>
              </div>
            </div>
            <div className='rounded-xl border border-border/40 bg-card p-4 shadow-sm'>
              <div className='flex items-center justify-between mb-3'>
                <div className='flex items-center gap-2'>
                  <span className='text-sm font-semibold'>{selectedDate ? fmt(selectedDate, 'M月d日 EEEE', { locale: zhCN }) : '日程'}</span>
                  {selectedDayRecords.length > 0 && <Badge variant='secondary' className='text-[10px]'>{selectedDayRecords.length} 条</Badge>}
                </div>
              </div>
            ...
            {recentRecords.length > 0 && (
              <div className='rounded-xl border border-border/40 bg-card p-4 shadow-sm'>
                <div className='flex items-center gap-2 mb-3'>
                  ...
                  <span className='text-xs font-semibold text-muted-foreground'>近期记录</span>
                </div>
                <div className='flex gap-3 overflow-x-auto pb-1'>
                  {recentRecords.map((record) => {
                    ...
                    const payment = isPayment ? (record as PaymentRecord) : null
                    return (
                      <div key={record.id} className={cn('flex shrink-0 items-center gap-2 rounded-full border px-3 py-1.5', colors.ring)}>
                        <div className={cn('h-2 w-2 rounded-full', colors.dot)} />
                        <span className='text-[11px] font-medium whitespace-nowrap'>{record.name}</span>
                        {payment && <span className={cn('text-[10px] font-semibold', colors.text)}>{payment.direction === 'income' ? '+' : '-'}{formatAmount(payment.amount, payment.currency)}</span>}
                      </div>
                    )
                  })}
                </div>
