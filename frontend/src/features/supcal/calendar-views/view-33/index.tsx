...
import { ChevronLeft, ChevronRight, CalendarDays, TrendingUp, TrendingDown, Clock, CheckCircle2, Flame, Trophy } from 'lucide-react'
...
export function CalendarView33() {
  ...
  const streakInfo = useMemo(() => {
    ...
    for (const day of days) {
      ...
      const hasRecs = (recordsByDate.get(dateKey) || []).length > 0
      if (hasRecs) {
        tempStreak++
    ...
    let activeDays = 0
    recordsByDate.forEach((recs) => { if (recs.length > 0) activeDays++ })
    return { currentStreak, maxStreak, activeDays, totalDays: days.length }
  }, [days, recordsByDate])
    ...
    <>
      ...
      <Main>
        <div className='mb-6 flex items-center justify-between'>
          <div>
            <h1 className='text-2xl font-bold tracking-tight'>日历视图 33 · 打卡统计</h1>
            <p className='mt-1 text-sm text-muted-foreground'>月历打卡标记 + 连续签到统计 + 完成率</p>
          </div>
        </div>
        {recordsQuery.isLoading ? (
          ...
          <div className='flex gap-6'>
              ...
              <div className='grid grid-cols-2 gap-2'>
                <div className='rounded-xl border border-amber-500/20 bg-amber-500/5 p-3 shadow-sm'>
                  <div className='flex items-center gap-1.5 mb-1'>
                    ...
                    <span className='text-[10px] text-amber-600 dark:text-amber-400 font-medium'>当前连续</span>
                  </div>
                  <p className='text-xl font-black text-amber-600 dark:text-amber-400'>{streakInfo.currentStreak}<span className='text-[10px] font-normal ml-0.5'>天</span></p>
                </div>
                <div className='rounded-xl border border-primary/20 bg-primary/5 p-3 shadow-sm'>
                  <div className='flex items-center gap-1.5 mb-1'>
                    <Trophy className='h-3.5 w-3.5 text-primary' />
                    <span className='text-[10px] text-primary font-medium'>最长连续</span>
                  </div>
                  <p className='text-xl font-black text-primary'>{streakInfo.maxStreak}<span className='text-[10px] font-normal ml-0.5'>天</span></p>
                </div>
              </div>
            ...
            <div className='min-w-0 flex-1'>
              <div className='mb-4 rounded-xl border border-border/60 bg-card px-4 py-3 shadow-sm'>
                <div className='flex items-center justify-between'>
                  <div className='flex items-center gap-3'>
                    <CheckCircle2 className='h-4 w-4 text-primary' />
                    <div>
                      <h3 className='text-sm font-semibold'>{selectedDate ? fmt(selectedDate, 'M月d日 EEEE', { locale: zhCN }) : ''}</h3>
                      <p className='text-[11px] text-muted-foreground'>完成率 {streakInfo.totalDays > 0 ? Math.round((streakInfo.activeDays / streakInfo.totalDays) * 100) : 0}%</p>
                    </div>
                  </div>
                ...
                <div className='mt-3 h-2 w-full rounded-full bg-muted/30'>
                  <div className='h-2 rounded-full bg-primary transition-all' style={{ width: `${streakInfo.totalDays > 0 ? (streakInfo.activeDays / streakInfo.totalDays) * 100 : 0}%` }} />
                </div>
              </div>
              {selectedDayRecords.length > 0 ? (
                  ...
                  {selectedDayRecords.map((record) => {
                    ...
                    return (
                      <button
                        ...
                        className={cn(
                          'flex w-full items-center gap-3 rounded-xl border-l-4 bg-card px-4 py-3 text-left shadow-sm transition-all hover:-translate-y-0.5 hover:shadow-md',
                          colors.ring
                        )}
                      >
                ...
                <div className='flex flex-col items-center justify-center rounded-xl border border-dashed border-border/40 py-16 text-muted-foreground'>
                  ...
                  <p className='text-sm'>当天暂无打卡记录</p>
