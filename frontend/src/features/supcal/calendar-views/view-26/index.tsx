...
import { ChevronLeft, ChevronRight, CalendarDays, TrendingUp, TrendingDown, Bell, BarChart3, Activity } from 'lucide-react'
...
export function CalendarView26() {
  ...
  const incomeCount = useMemo(() => allMonthRecords.filter((r) => r.type === 'payment' && (r as PaymentRecord).direction === 'income').length, [allMonthRecords])
  const expenseCount = useMemo(() => allMonthRecords.filter((r) => r.type === 'payment' && (r as PaymentRecord).direction === 'expense').length, [allMonthRecords])
  const reminderCount = useMemo(() => allMonthRecords.filter((r) => r.type === 'simple').length, [allMonthRecords])
  const daysWithRecords = useMemo(() => {
    let count = 0
    recordsByDate.forEach((recs) => { if (recs.length > 0) count++ })
    return count
  }, [recordsByDate])
    ...
    <>
      ...
      <Main>
        <div className='mb-6 flex items-center justify-between'>
          <div>
            <h1 className='text-2xl font-bold tracking-tight'>日历视图 26 · 迷你统计</h1>
            <p className='mt-1 text-sm text-muted-foreground'>紧凑月历网格 + 四维统计卡片 + 数字角标</p>
          </div>
        </div>
        {recordsQuery.isLoading ? (
          <div className='space-y-3'>
            ...
            <div className='grid grid-cols-4 gap-3'>
              {[0, 1, 2, 3].map((i) => <div key={i} className='h-24 animate-pulse rounded-xl bg-muted/30' />)}
            </div>
          ...
          <div className='space-y-4'>
            ...
            <div className='grid grid-cols-4 gap-3'>
              <div className='rounded-xl border border-emerald-500/20 bg-card p-3 shadow-sm'>
                <div className='flex items-center justify-between'>
                  ...
                  <Badge className='bg-emerald-500/15 text-emerald-700 dark:text-emerald-400 text-[9px] h-5'>{incomeCount}</Badge>
                </div>
                <p className='mt-2 text-[10px] text-muted-foreground'>总收入</p>
              ...
              <div className='rounded-xl border border-rose-500/20 bg-card p-3 shadow-sm'>
                <div className='flex items-center justify-between'>
                  ...
                  <Badge className='bg-rose-500/15 text-rose-700 dark:text-rose-400 text-[9px] h-5'>{expenseCount}</Badge>
                </div>
                <p className='mt-2 text-[10px] text-muted-foreground'>总支出</p>
              ...
              <div className='rounded-xl border border-blue-500/20 bg-card p-3 shadow-sm'>
                <div className='flex items-center justify-between'>
                  ...
                  <Badge className='bg-blue-500/15 text-blue-700 dark:text-blue-400 text-[9px] h-5'>{reminderCount}</Badge>
                </div>
                <p className='mt-2 text-[10px] text-muted-foreground'>提醒</p>
                <p className='text-sm font-bold text-blue-700 dark:text-blue-400'>{reminderCount} 条</p>
              </div>
              <div className='rounded-xl border border-primary/20 bg-card p-3 shadow-sm'>
                <div className='flex items-center justify-between'>
                  <BarChart3 className='h-4 w-4 text-primary' />
                  <Badge className='bg-primary/15 text-primary text-[9px] h-5'>{daysWithRecords}</Badge>
                </div>
                <p className='mt-2 text-[10px] text-muted-foreground'>活跃天数</p>
                <p className='text-sm font-bold text-primary'>{daysWithRecords}/{days.length} 天</p>
              </div>
            </div>
            <div className='rounded-xl border border-border/40 bg-card p-4 shadow-sm'>
              ...
              <div className='grid grid-cols-7 gap-1'>
                {Array.from({ length: firstDayOffset }).map((_, i) => <div key={`e-${i}`} className='h-8' />)}
                {days.map((day) => {
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        'relative flex h-8 w-full flex-col items-center justify-center rounded-md text-[11px] transition-all',
                    ...
                    >
                      {fmt(day, 'd')}
                      {recCount > 0 && !isSelected && (
                        <div className='absolute bottom-0.5 left-1/2 -translate-x-1/2'>
                          <div className='h-0.5 rounded-full bg-primary' style={{ width: `${Math.max(4, Math.min(16, recCount * 4))}px` }} />
                        </div>
                      )}
                    </button>
                  )
            ...
            <div className='rounded-xl border border-border/60 bg-card px-4 py-3 shadow-sm'>
              <div className='flex items-center justify-between mb-3'>
                <div className='flex items-center gap-2'>
                  ...
                  <span className='text-sm font-semibold'>{selectedDate ? fmt(selectedDate, 'M月d日 EEEE', { locale: zhCN }) : ''}</span>
                </div>
