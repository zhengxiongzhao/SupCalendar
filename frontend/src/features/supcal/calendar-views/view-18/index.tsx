...
import { ChevronLeft, ChevronRight, CalendarDays, TrendingUp, TrendingDown, Wallet, PanelLeft } from 'lucide-react'
...
export function CalendarView18() {
  ...
  const selectedDayIncome = useMemo(() => selectedDayRecords.reduce((s, r) => r.type === 'payment' && (r as PaymentRecord).direction === 'income' ? s + (r as PaymentRecord).amount : s, 0), [selectedDayRecords])
  const selectedDayExpense = useMemo(() => selectedDayRecords.reduce((s, r) => r.type === 'payment' && (r as PaymentRecord).direction === 'expense' ? s + (r as PaymentRecord).amount : s, 0), [selectedDayRecords])
    ...
    <>
      ...
      <Main>
        <div className='mb-6 flex items-center justify-between'>
          <div>
            <h1 className='text-2xl font-bold tracking-tight'>日历视图 18 · 侧栏导航</h1>
            <p className='mt-1 text-sm text-muted-foreground'>左侧迷你月历 + 右侧全宽日程详情</p>
          </div>
        </div>
        {recordsQuery.isLoading ? (
          <div className='flex gap-4'>
            <div className='w-[260px] shrink-0 space-y-3'><div className='h-10 animate-pulse rounded-xl bg-muted/30' /><div className='h-56 animate-pulse rounded-xl bg-muted/30' /><div className='h-24 animate-pulse rounded-xl bg-muted/30' /></div>
            <div className='flex-1 space-y-3'><div className='h-12 animate-pulse rounded-xl bg-muted/30' /><div className='h-80 animate-pulse rounded-xl bg-muted/30' /></div>
          </div>
        ) : (
          <div className='flex gap-4'>
            <div className='w-[260px] shrink-0 space-y-3'>
              ...
              <div className='rounded-xl border border-border/40 bg-card p-2.5 shadow-sm'>
                ...
                <div className='grid grid-cols-7 gap-0.5'>
                  ...
                  {days.map((day) => {
                    ...
                    return (
                      <button key={dateKey} onClick={() => handleDaySelect(day)} className={cn('relative flex h-7 w-full items-center justify-center rounded text-[10px] transition-all', isSelected ? 'bg-primary text-primary-foreground font-bold' : isCurrentDay ? 'bg-primary/10 text-primary font-semibold' : 'hover:bg-accent/40', isWeekend(day) && !isSelected && 'text-rose-600/50')}>
                        {fmt(day, 'd')}
              ...
              <Button variant='ghost' size='sm' className='w-full h-7 text-[10px]' onClick={() => setCurrentDate(new Date())}>回到今天</Button>
              ...
              <div className='rounded-xl border border-border/60 bg-card px-5 py-4 shadow-sm'>
                <div className='flex items-center justify-between'>
                  <div className='flex items-center gap-3'>
                    ...
                    <div>
                      <h3 className='text-base font-semibold'>{selectedDate ? fmt(selectedDate, 'yyyy年M月d日 EEEE', { locale: zhCN }) : '请选择日期'}</h3>
                      {selectedDate && selectedDayRecords.length > 0 && <p className='text-xs text-muted-foreground'>{selectedDayRecords.length} 条记录</p>}
                    </div>
                  </div>
                  {selectedDate && (
                    <div className='flex items-center gap-3'>
                      {selectedDayIncome > 0 && <span className='text-xs font-bold text-emerald-600 dark:text-emerald-400'>+{formatAmount(selectedDayIncome, 'CNY')}</span>}
                      {selectedDayExpense > 0 && <span className='text-xs font-bold text-rose-600 dark:text-rose-400'>-{formatAmount(selectedDayExpense, 'CNY')}</span>}
                    </div>
                  )}
                </div>
              </div>
